 ...
 */
export function createTelnetConfig(
 ...
 */
export function getGameConstantsValidationErrors(
  constants: GameNetworkingConstants
): string[] {
  const errors: string[] = []
  if (constants.maxConnections <= 0 || constants.maxConnections > 10000) {
    errors.push(
      `maxConnections must be between 1 and 10000, got ${constants.maxConnections}`
    )
  }
  if (
    constants.maxConnectionsPerIP <= 0 ||
    constants.maxConnectionsPerIP > 100
  ) {
    errors.push(
      `maxConnectionsPerIP must be between 1 and 100, got ${constants.maxConnectionsPerIP}`
    )
  }
  if (constants.defaultPort <= 1024 || constants.defaultPort > 65535) {
    errors.push(
      `defaultPort must be between 1025 and 65535, got ${constants.defaultPort}`
    )
  }
  if (constants.idleTimeoutMs < 60000 || constants.idleTimeoutMs > 7200000) {
    errors.push(
      `idleTimeoutMs must be between 60000 and 7200000, got ${constants.idleTimeoutMs}`
    )
  }
  if (constants.maxInputLength < 256 || constants.maxInputLength > 32768) {
    errors.push(
      `maxInputLength must be between 256 and 32768, got ${constants.maxInputLength}`
    )
  }
  return errors
}

export function validateGameConstants(
  constants: GameNetworkingConstants
): void {
  const errors = getGameConstantsValidationErrors(constants)
  if (errors.length > 0) {
