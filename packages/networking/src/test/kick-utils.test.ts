...
import {
  getClientsToKickByIP,
  kickExcessConnections
} from '../telnet/kick-utils'

function makeSocket(ip: string): net.Socket {
  return { remoteAddress: ip } as unknown as net.Socket
}
...
describe('kickExcessConnections', () => {
  it('kicks the newest excess clients and emits disconnect', () => {
    ...
    expect((disconnect as Mock).mock.calls.map(([id]) => id)).toEqual([
      'c2',
      'c3'
    ])
